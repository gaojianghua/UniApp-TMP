	...
	if(data.updateType == 'forcibly' || data.updateType == 'solicit'){
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				popupObj.change({
					progressValue: 100,
					progressTip: i18n.t("正在安装文件..."),
					progress: true,
					buttonNum: 0
				});
				plus.runtime.install(download.filename, {}, function() {
					popupObj.change({
						contentText: i18n.t("应用资源更新完成！"),
						buttonNum: 1,
						progress: false
					});
				}, function(e) {
					popupObj.cancel();
					plus.nativeUI.alert(i18n.t("安装文件失败") + "[" + e.code + "]：" + e.message);
				});
			} else {
				popupObj.change({
					contentText: i18n.t("文件下载失败..."),
					buttonNum: 1,
					progress: false
				});
			}
				...
				case 3:
					...
					if(progress - lastProgressValue >= 2){
						...
						popupObj.change({
							...
							progressTip: i18n.t("已下载") + progress + "%",
							progress: true
						});
					}
					break;
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				...
				plus.nativeUI.alert(i18n.t("文件下载失败..."));
			}
		});
		dtask.start();
	}
...
function updatePopup(data, callback) {
	...
	let popupViewContentList = [{
		...
		},
		{
			tag: 'font',
			id: 'title',
			text: i18n.t("发现新版本") + ' ' + data.versionName,
			textStyles: {
				size: '18px',
				color: "#333",
				weight: "bold",
...
function downloadPopupDrawing(data){
	...
	let progressTip = data.progressTip || i18n.t("准备下载...");
	let contentText = data.contentText || i18n.t("正在为您更新，请耐心等待");
	let elementList = [
		{
			tag: 'rect', //背景色
			color: '#FFFFFF',
