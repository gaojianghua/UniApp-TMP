<template>
	<view class="page">
		<m-navbar bgColor="#fff" textColor="#fb7299" value="page.消息列表" i18n></m-navbar>
		<!-- 内容区域 -->
		<m-scroll-y :isLoading="isLoading" i18n :scrollStyle="scrollStyle" :load="load" @loadmore="loadmore" bgColor="transparent"
			...
			<view class="px-2">
				<view v-if="list.length != 0" class="item bg-white rounded-1 d-flex a-center j-sb p-2 mt-2"
					v-for="(item, i) in list" :key="i" @click="$tools.Navigate.navigateTo('/pages-offspring/message-detail/index', item.id)">
