<template>
	<view class="page">
		...
		<m-scroll :isLoading="isLoading" i18n :scrollStyle="scrollStyle" :load="load" bgColor="transparent"
			...
			<view v-if="$store.state.orderHistoryList && $store.state.orderHistoryList.length != 0 && !isSearchResult"
				...
				<view class="history-list d-flex flex-wrap">
					<view class="history-item px-2 mt-2 mr-2 main-bg-color text-white"
						v-for="(item, i) in $store.state.orderHistoryList" :key="i" @click="searchHistoryItem(item)">
						{{item}}
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 开始搜索
			startSearch(e) {
				if (!this.$store.state.orderHistoryList.includes(e) && e) {
					let arr = this.$store.state.orderHistoryList
					arr.push(e)
					this.$store.commit('updateOrderHistoryList', arr)
					uni.setStorageSync('orderHistoryList', arr)
				}
			...
			// 清空历史
			clearHistory() {
				this.$store.commit('updateOrderHistoryList', [])
				uni.setStorageSync('orderHistoryList', [])
			},
			// 进入订单详情页
			openDetail(i) {
