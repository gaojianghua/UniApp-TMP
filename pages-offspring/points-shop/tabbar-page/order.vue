		...
		<view class="px-2">
			<view v-if="list.length != 0" class="item bg-white rounded-1 d-flex a-center j-sb p-2 mt-2"
				...
				<view class="btns border-2 rounded-1 px-3"
					:class="item.state == 0 ? 'text-warning border-warning' : (item.state == 1 ? 'text-success border-success' : 'text-muted border-muted')">
					{{item.state == 0 ? $t('配送中') : (item.state == 1 ? $t('已送达') : $t('已完成'))}}
				</view>
			</view>
		</view>
	</m-scroll-y>
